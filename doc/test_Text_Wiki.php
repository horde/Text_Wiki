...
$plist = ['Default', 'BBCode', 'Cowiki', 'Doku', 'Mediawiki', 'Tiki', 'X-choice', 'Creole'];
$rlist = ['Xhtml', 'Plain', 'Latex', 'Cowiki', 'Doku', 'Tiki', 'Ooosxw', 'Pdf', 'Docbook', 'Creole'];
 ...
 */
if (in_array(php_sapi_name(), ['cli', 'cgi'])) {
    $html = false;
    $parser = $_SERVER['argv'][1] ?? 'BBCode';
    $render = $_SERVER['argv'][2] ?? 'Xhtml';
    ...
    foreach (['parser' => $plist[0], 'render' => $rlist[0],
        'exchoice' => ($elist ? $elist[0] : ''), 'source' => ''] as $fld => $def) {
