...
/**
 * Tiki round-trip idempotency test
 *
 ...
 * @coversNothing
 */
class TikiIdempotencyTest extends TestCase
{
    private TikiParser $parser;
    private Tiki $renderer;

    protected function setUp(): void
    {
        $this->parser = new TikiParser();
        $this->renderer = new Tiki();
    }
    ...
    {
        $this->assertIdempotent('__bold text__', 'Bold');
    }
    ...
    {
        $this->assertIdempotent('===underlined===', 'Underline');
    }
    ...
    public function testColortextIdempotency(): void
    {
        $this->assertIdempotent('~~red:colored text~~', 'Colortext');
    }
    ...
    public function testCodeBlockWithLanguageIdempotency(): void
    {
        $this->assertIdempotent("{CODE(lang=php)}\necho hello\n{CODE}", 'Code block with language');
    }

    public function testRawIdempotency(): void
    {
        $this->assertIdempotent('~np~__not bold__ not parsed~/np~', 'Raw noop');
    }
    ...
    {
        $this->assertIdempotent(
            '{img src="http://example.com/img.png" alt="Photo"}',
            'Image'
        );
    }
    ...
    {
        $this->assertSame('tiki', $this->renderer->getFormat());
    }
}
