...
/**
 * Integration test: Creole markup -> Parser -> AST -> Xhtml Renderer
 *
 * Tests the complete pipeline with modern typed AST.
 * @coversNothing
 */
class CreoleIntegrationTest extends TestCase
{
    ...
    public function testHeadingWithTrailingEquals(): void
    {
        $doc = $this->parser->parse('== Heading 2 ==');
        $html = $this->renderer->render($doc);

        $this->assertStringContainsString('<h2>', $html);
        $this->assertStringContainsString('Heading 2', $html);
    ...
    public function testEscapeChar(): void
    {
        $doc = $this->parser->parse('This is ~*not bold~*');
        $html = $this->renderer->render($doc);
    ...
    {
        $this->assertSame('creole', $this->parser->getFormat());
    }
}
