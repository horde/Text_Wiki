        ...
        if (count($arr) == 1) {
            ...
            foreach ($arr as $r) {
                if (($i % 2) == 0) {
                    ...
                    if (strlen($r) == 2) {
                        if ($state === 'i') {
                            $output .= $this->wiki->addToken($this->rule, ['type' => 'end']);
                            $state = '';
                        } elseif ($state === 'bi') {
                            $output .= $this->wiki->addToken($this->rule, ['type' => 'end']);
                            $state = 'b';
                        } elseif ($state === 'ib') {
                            $output .= $this->wiki->addToken('Strong', ['type' => 'end']);
                            $output .= $this->wiki->addToken($this->rule, ['type' => 'end']);
                            $output .= $this->wiki->addToken('Strong', ['type' => 'start']);
                            $state = 'b';
                        } elseif ($state === 'both') {
                            $output .= $this->wiki->addToken('Strong', ['type' => 'start']);
                            $output .= $this->wiki->addToken($this->rule, ['type' => 'start']);
                            $output .= $buffer;
                            $output .= $this->wiki->addToken($this->rule, ['type' => 'end']);
                            $state = 'b';
                        } else {
